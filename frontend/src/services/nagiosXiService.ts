...
export class NagiosXIService {
  ...
  static async authenticate(
    ...
    try {
      const response = await fetch(
        `${instance.url}/nagiosxi/api/v1/system/status`,
        {
          headers: {
  ...
  static async authenticateWithStoredCredentials(
    instance: any,
  ): Promise<boolean> {
    try {
      ...
      if (storedCredentials) {
        ...
        const credentials = JSON.parse(
          decodeURIComponent(atob(storedCredentials)),
        );
  ...
  static async getSystemInfo(instance: any): Promise<SystemInfo> {
    try {
      ...
      const isAuthenticated =
        await this.authenticateWithStoredCredentials(instance);
